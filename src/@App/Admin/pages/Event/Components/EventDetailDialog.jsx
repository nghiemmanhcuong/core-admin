		...
		>
			...
			<TableBody>
				...
				<TableRow>
					<TableCell>
						<Typography color="primary" variant="subtitle1">
							{t('label.reception_start_edit')}
						</Typography>
					</TableCell>
					<TableCell>{data?.reception_start}</TableCell>
					<TableCell>
						<Typography color="primary" variant="subtitle1">
							{t('label.reception_end')}
						</Typography>
					</TableCell>
				...
				<TableRow>
					...
					<TableCell colSpan={3}>
						{data?.tag?.length > 0 && Array.isArray(data?.tag)
							? data?.tag?.map((item, index) => {
									return (
										<span key={index} className="bg-grey-300 p-4 rounded-4">
											{item}
										</span>
									)
							  })
				...
				<TableRow>
					...
					<TableCell colSpan={3}>
						{data?.category?.length > 0 && Array.isArray(data?.category)
