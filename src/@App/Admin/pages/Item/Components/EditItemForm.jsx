...
const EditItemForm = props => {
	...
	const filterData = (exchange_area) => {
		const exValue = dataArea.filter(item=>{
			return item.label === exchange_area
		})
		if(exValue) return exValue[0].value
	}
	...
	const {
		...
		defaultValues: {
			...
			exchange_area: itemData?.exchange_area ? filterData(itemData?.exchange_area) : null,
			...
			Object.keys(data).forEach(key => {
				...
				if (data[key]) {
					...
					if (key === 'available_end') {
						return formData.append('available_end', moment(data?.available_end).format('YYYY-MM-DD'))
					}
